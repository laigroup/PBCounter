...
void printDd(const Cudd &mgr, const ADD &dd, int n, int pr) {
  ...
  // prints to standard output a DD and its statistics: number of nodes, number of leaves, number of minterms
  Cudd_PrintDebug(gbm, ddNode, n, pr);                          
}
...
void writeDd(const Cudd &mgr, const ADD &dd, const string &filePath) {
  ...
  Cudd_DumpDot(gbm, 1, ddnodearray, NULL, NULL, outfile);   // dumps function to .dot fil
...
void mainVisual(int argc, char *argv[]) {
  ...
  ADD tmp1, tmp2;
  tmp1 = dd1.Compose(mgr.addOne(), 3);
  tmp2 = dd1.Compose(mgr.addZero(), 3);
  ...
  // writeDd(mgr, tmp1, DOT_DIR + "tmp1.dot");
  // writeDd(mgr, tmp2, DOT_DIR + "tmp2.dot");

  ADD One = mgr.addOne();
  ADD Zero = mgr.addZero();
  writeDd(mgr, Zero, DOT_DIR + "ddZero.dot");
}
